...
* **內容：**
    ...
    * **標籤定義：** 將熱門度評分離散化為五類：`FAILED（爆死 ~~拉完了~~）` / `NICHE（小眾）` / `AVERAGE（一般）` / `POPULAR（熱門）` / `PHENOMENAL（現象級 ~~夯~~）`。
...
* **備註：**
    ...
    * Steam 本身並沒有公開每個作品的願望清單數，對此 SteamDB 使用的方案是顯示「願望清單數__排名__」以及額外添加「__追蹤者__數量」用於參考。
